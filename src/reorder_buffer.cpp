...
reorder_buffer::reorder_buffer(sc_module_name name,unsigned int sz,unsigned int pht_size, unsigned int pred_size, unsigned int buffer_size, int flag_mode, nana::listbox &gui, nana::listbox::cat_proxy instr_gui): 
sc_module(name),
tam(sz),
flag_mode(flag_mode),
preditor(pred_size),
branch_prediction_buffer(buffer_size, pred_size),
bcp(pht_size, pred_size),
gui_table(gui),
...
void reorder_buffer::leitura_issue()
{
    ...
    while(true)
    {
        ...
        else if(ord[0].at(0) == 'B')
        {
            ...
            //se escolhido no menu bpb, vai para o primeiro else
            //se escolhido no menu bcp, vai para o segundo else
            if(flag_mode == 1){
                ...
                ptrs[pos]->prediction = bcp.bcp_predict(ptrs[pos]->pc);
            }
...
branch_correlation_predictor reorder_buffer::get_bcp() {
    return bcp;
}
